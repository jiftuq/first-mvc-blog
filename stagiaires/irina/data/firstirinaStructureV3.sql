...
-- -----------------------------------------------------
-- Table `firstirina`.`category`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `firstirina`.`category` ;

CREATE TABLE IF NOT EXISTS `firstirina`.`category` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `title` VARCHAR(45) CHARACTER SET 'cp1250' NOT NULL,
...
-- -----------------------------------------------------
-- Table `firstirina`.`category_has_article`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `firstirina`.`category_has_article` ;

CREATE TABLE IF NOT EXISTS `firstirina`.`category_has_article` (
  `category_id` INT UNSIGNED NOT NULL,
  `article_id` INT UNSIGNED NOT NULL,
  PRIMARY KEY (`category_id`, `article_id`),
  CONSTRAINT `fk_category_has_article_category1`
    FOREIGN KEY (`category_id`)
    REFERENCES `firstirina`.`category` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `fk_category_has_article_article1`
    FOREIGN KEY (`article_id`)
    REFERENCES `firstirina`.`article` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB;

CREATE INDEX `fk_category_has_article_article1_idx` ON `firstirina`.`category_has_article` (`article_id` ASC) VISIBLE;

CREATE INDEX `fk_category_has_article_category1_idx` ON `firstirina`.`category_has_article` (`category_id` ASC) VISIBLE;
